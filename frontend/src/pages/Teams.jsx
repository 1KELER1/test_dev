...
export const Teams = ({ username }) => {
    ...
    const fetchTeams = async () => {
      try {
        ...
        if (
          ...
          const ownTeams = response.own.map((team) => ({
            ...team,
            isOwner: true,
          }));
          const memberTeams = response.member.map((team) => ({
            ...team,
            isOwner: false,
          }));
  ...
  const createNewTeam = async (name, description) => {
    try {
      const response = await createTeam({ name, description, members: [] });
      if (response && response.data) {
        setTeams((prevTeams) => [
          { ...response.data, owner: username, isOwner: true },
          ...prevTeams,
        ]);
      } else {
    ...
    <div className="h-screen w-screen flex bg-gray-50 ">
      ...
      <div className="h-full w-full p-7 bg-gray-50 flex flex-col">
        ...
        <div className="stats flex flex-row mt-7 mb-7 items-end space-x-4">
          ...
          <Button color="green" size="lg" onClick={() => setModalIsOpen(true)}>
            Создать команду
          </Button>
        </div>
        <CreateTeamModal
